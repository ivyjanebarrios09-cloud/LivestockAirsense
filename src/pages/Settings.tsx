    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6 pb-28">
      ...
      {!user ? (
        ...
        <div className="space-y-6">
          ...
          {/* Environmental Alarm Thresholds Section */}
          <section className="bg-system-panel border border-system-border shadow-sm rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-system-border bg-system-bg flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Sliders className="w-4 h-4 text-system-accent" />
                <h3 className="font-bold text-sm tracking-tight text-system-text uppercase font-mono">Critical Safety Limits</h3>
              </div>
            ...
            <div className="p-5 md:p-6 space-y-6">
              <p className="text-xs text-system-muted leading-relaxed">
                Configure upper hazard boundaries. Exceeding these levels will immediately initiate visual alerts on the active Dashboard, and log automatic emergency events for critical actions.
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                
                {/* Temp Slider */}
                <div className="space-y-2 bg-system-bg border border-system-border p-4 rounded-xl">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-semibold text-system-text">Max Temperature limit</span>
                    <span className="font-mono font-bold text-system-accent">{localTempMax} °C</span>
                  </div>
                  <input 
                    type="range" 
                    min="18" 
                    ...
                    step="1"
                    value={localTempMax} 
                    onChange={(e) => setLocalTempMax(Number(e.target.value))}
                    className="w-full accent-system-accent cursor-pointer" 
                  />
                  <div className="flex justify-between text-[10px] text-system-muted font-mono">
                    <span>18°C (Cool)</span>
                ...
                <div className="space-y-2 bg-system-bg border border-system-border p-4 rounded-xl">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-semibold text-system-text">Max Humidity Limit</span>
                    <span className="font-mono font-bold text-system-accent">{localHumidityMax} %</span>
                  </div>
                  <input 
                    type="range" 
                    min="50" 
                    max="90" 
                    step="5"
                    value={localHumidityMax} 
                    onChange={(e) => setLocalHumidityMax(Number(e.target.value))}
                    className="w-full accent-system-accent cursor-pointer" 
                  />
                  <div className="flex justify-between text-[10px] text-system-muted font-mono">
                    <span>50% (Dry)</span>
                ...
                <div className="space-y-2 bg-system-bg border border-system-border p-4 rounded-xl">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-semibold text-system-text">Max CO2 Limit</span>
                    <span className="font-mono font-bold text-system-accent">{localCo2Max} ppm</span>
                  </div>
                  <input 
                    type="range" 
                    min="500" 
                    ...
                    step="50"
                    value={localCo2Max} 
                    onChange={(e) => setLocalCo2Max(Number(e.target.value))}
                    className="w-full accent-system-accent cursor-pointer" 
                  />
                  <div className="flex justify-between text-[10px] text-system-muted font-mono">
                    <span>500 ppm</span>
                ...
                <div className="space-y-2 bg-system-bg border border-system-border p-4 rounded-xl">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-semibold text-system-text">Max Ammonia (NH3) Limit</span>
                    <span className="font-mono font-bold text-system-accent">{localAmmoniaMax.toFixed(1)} ppm</span>
                  </div>
                  <input 
                    type="range" 
                    min="1.0" 
                    max="8.0" 
                    step="0.5"
                    value={localAmmoniaMax} 
                    onChange={(e) => setLocalAmmoniaMax(Number(e.target.value))}
                    className="w-full accent-system-accent cursor-pointer" 
                  />
                  <div className="flex justify-between text-[10px] text-system-muted font-mono">
                    <span>1.0 ppm (Pristine)</span>
                    <span>8.0 ppm (Hazards)</span>
                  </div>
                </div>
